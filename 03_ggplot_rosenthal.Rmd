...
#1a  Using the an approximate method, provide a rough 95% confidience interval for the mean.

First, we need to find the standard deviation of the number of genes regulated by a regulatory gene. 
...
#1b provide an interpretaton of this result.
...
##18a. What is the mean and standard deviation?

these are straightforward to calculate, we can use the `mean()` and `sd()` functions.
```{r}
mean(beetles)
sd(beetles)
```

##18b What is the standard error?
using the same formula described above, we can calculate standard error as follows:
```{r}
std_error <- sd(beetles)/(sqrt(length(beetles))) #sd/sqrt of number of samples
std_error
```

##18c Find the 95% confidence interval
...
##18e 
I would not expect the standard deviation to change in a predictable way with a larger sample size, as it describes the spread of the data. If the data is spread out (or highly variable), the standard deviation will be large no matter the sample size. I would, however, expect my standard deviation to be more precise as my sample size increases.
...
ice <- ice %>%
  ...
  arrange(Month)
ice <- ice %>%
  #convert Month_Name to a factor and put it in order
...
```{r}
ice_min <- ice %>%
  #group by year so we can find the minimum of each year
group_by(Year) %>%
...
```{r}
ice <- ice %>%
#This creates 4 equal sized bins of months
mutate(Season_No = cut_interval(Month, n = 4))
#This renames them to meaningful things, eg season names, and doesn't work
#revalue(Season_No, c("[1,3.75]" = "Winter", "(3.75,6.5]" = "Spring", "(6.5,9.25]" = "Summer", "(9.25,12]" = "Fall" ))
```
